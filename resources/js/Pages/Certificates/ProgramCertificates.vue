...
const fetchData = async () => {
    ...
    try {
        const [programResponse, certificateResponse] = await Promise.all([
            axios.get(`/api/programs/${props.programId}`),
            axios.get(`/api/programs/${props.programId}/certificates`),
        ]);
        program.value = programResponse.data?.data ?? null;
        ...
        const message =
            ...
            "Unable to load program certificates.";
        toast.error(message);
    } finally {
...
<template>
    <Head title="Program Certificates" />
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                <div v-else-if="certificates.length === 0" class="py-10 text-center text-sm text-gray-500">
                    No certificates issued for this program.
                </div>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <tr v-for="certificate in certificates" :key="certificate.id">
                                ...
                                <td class="px-4 py-3 text-gray-600">
                                    {{ certificate.session?.title || "Program Certificate" }}
                                </td>
                                <td class="px-4 py-3">
                                    <span
