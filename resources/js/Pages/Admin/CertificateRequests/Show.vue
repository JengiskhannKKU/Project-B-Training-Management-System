...
const fetchRequest = async () => {
    ...
    try {
        const response = await axios.get(
            `/api/admin/certificate-requests/${props.id}`
        );
        ...
        toast.error(message);
        router.visit("/admin/certificate-requests");
    } finally {
...
const approveRequest = async () => {
    ...
    try {
        ...
        const response = await axios.post(
            `/api/admin/certificate-requests/${props.id}/approve`
        );
...
const handleReject = async (note) => {
    try {
        await ensureCsrf();
        await axios.post(`/api/admin/certificate-requests/${props.id}/reject`, {
            note,
        });
...
const getTrainerName = () => {
    if (request.value?.type === "session") {
        return request.value?.session?.trainer?.name || "—";
    }
    return request.value?.program?.creator?.name || "—";
};
    ...
    <AdminLayout>
        <div class="space-y-6">
            ...
            <div v-else-if="request" class="space-y-6">
                ...
                <!-- Trainer Information (Admin Only) -->
                <div class="rounded-lg border border-gray-200 bg-white p-6">
                    ...
                    <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div v-if="request.type === 'session' && request.session?.trainer">
                            <dt class="text-sm font-medium text-gray-500">Email</dt>
                            <dd class="mt-1 text-sm text-gray-900">
                                {{ request.session.trainer.email || "—" }}
                            </dd>
                        </div>
                        <div v-if="request.type === 'program' && request.program?.creator">
                            <dt class="text-sm font-medium text-gray-500">Email</dt>
                            <dd class="mt-1 text-sm text-gray-900">
                                {{ request.program.creator.email || "—" }}
                            </dd>
                        </div>
                    </dl>
                </div>
