    ...
    <Teleport to="body">
        <div v-if="show"
            ...
            <div class="relative max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white shadow-2xl flex flex-col"
                style="will-change: transform; contain: layout style paint;" @click.stop>

                <!-- Sticky Header -->
                <div
                    class="sticky top-0 z-20 border-b border-gray-100 bg-white/95 px-8 py-5 backdrop-blur flex items-center justify-between">
                    <div>
                        <h2 class="text-2xl font-bold text-gray-900 tracking-tight">{{ isEditMode ? 'Edit Program' :
                            'Create New Program' }}</h2>
                        <p class="text-sm text-gray-500 mt-0.5">Fill in the details to configure your course structure.
                        </p>
                    </div>
                    <button @click="handleClose"
                        class="rounded-full p-2 hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors">
                        <X :size="24" />
                    </button>
                </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <div v-if="!imagePreview && !props.course?.thumbnail_path && !thumbnailPath"
                                    ...
                                    <div
                                        class="p-4 rounded-full bg-white shadow-sm group-hover:scale-110 transition-transform mb-3">
                                        <UploadCloud class="text-teal-600" :size="24" />
                                    </div>
                                    <span class="text-sm font-medium text-gray-600 group-hover:text-teal-700">Click to
                                        upload</span>
                                    <span class="text-xs text-gray-400 mt-1">PNG, JPG up to 2MB</span>
                                </div>

                                <div v-else
                                    class="relative aspect-video w-full rounded-xl overflow-hidden border border-gray-200 shadow-sm group">
                                    <img :src="imagePreview || thumbnailPath || props.course?.thumbnail_path"
                                        class="w-full h-full object-cover" />
                                    <div
                                        class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                        <button type="button" @click="triggerFileInput"
                                            class="p-2 bg-white/20 hover:bg-white/30 backdrop-blur rounded-full text-white transition-colors"
                                            title="Change">
                                            <UploadCloud :size="18" />
                                        </button>
                                        <button type="button" @click="removeImage"
                                            class="p-2 bg-red-500/80 hover:bg-red-600 backdrop-blur rounded-full text-white transition-colors"
                                            title="Remove">
                                            <Trash2 :size="18" />
                                        </button>
                                    </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <div v-if="form.status === 'draft'" class="text-xs text-gray-600 mt-1">
                                    💾 Course saved but not active. Use this when details are still uncertain or subject
                                    to change.
                                </div>
                                ...
                                <div v-if="form.status === 'archived' && isEditMode" class="text-xs text-gray-600 mt-1">
                                    📦 Course stored for records. Used for completed courses or courses you want to keep
                                    archived.
                                </div>
                            </div>
                        ...
                        <div class="lg:col-span-2 space-y-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <TextInput v-model="form.title" type="text" class="w-full"
                                    placeholder="e.g. Mastering Vue.js Architecture" maxlength="50" />
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                    <label v-for="lvl in levels" :key="lvl.id"
                                        ...
                                        <div class="flex justify-between items-start mb-1">
                                            <span class="font-bold text-sm"
                                                :class="form.level === lvl.id ? 'text-teal-800' : 'text-gray-900'">
                                                {{ lvl.label }}
                                            </span>
                                            <CheckCircle2 v-if="form.level === lvl.id" :size="16"
                                                class="text-teal-600" />
                                        </div>
                    ...
                    <div class="rounded-2xl border border-gray-200 bg-gray-50/50 p-6 space-y-6"
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <textarea v-model="form.learning_outcomes" rows="4"
                                    class="w-full rounded-xl border-gray-300 focus:border-teal-500 focus:ring-teal-500 shadow-sm text-sm"
                                    placeholder="• Understand the core concepts..." maxlength="500"></textarea>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <textarea v-model="form.target_audience" rows="3"
                                        class="w-full rounded-xl border-gray-300 focus:border-teal-500 focus:ring-teal-500 shadow-sm text-sm"
                                        placeholder="Who is this course designed for?" maxlength="500"></textarea>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <textarea v-model="form.prerequisites" rows="3"
                                        class="w-full rounded-xl border-gray-300 focus:border-teal-500 focus:ring-teal-500 shadow-sm text-sm"
                                        placeholder="Required skills or equipment..." maxlength="500"></textarea>
                            ...
                            <div class="space-y-2">
                                ...
                                <textarea v-model="form.additional_info" rows="3"
                                    class="w-full rounded-xl border-gray-300 focus:border-teal-500 focus:ring-teal-500 shadow-sm text-sm"
                ...
                <div class="sticky bottom-0 z-20 border-t border-gray-100 bg-white px-8 py-4 flex justify-end gap-3">
                    ...
                    <PrimaryButton @click="handleSubmit" :class="{ 'opacity-25': form.processing }"
                        :disabled="form.processing" class="bg-[#2f837d] hover:bg-[#26685f]">
